...
app.get('/getBookInfo/:id', (req, res) => {
  ...
  for (var i=0; i < groceriesInfoJSON.length; i++){
    if(groceriesInfoJSON[i]["bookName"] == bookName) result = groceriesInfoJSON[i];
  }
  res.json({result});
});
...
app.get('/getBookInfoBasedOnId/:id', (req, res) => {
  ...
  var result = null;  // This variable will tell if we have a book or not.
  for (var i=0; i < groceriesInfoJSON.length; i++){
    if(groceriesInfoJSON[i]["id"] == bookId) result = groceriesInfoJSON[i];
  }
  res.json({result});
});
...
app.post('/addBook', (req, res) => {
  ...
  groceriesInfoJSON.push({id: uuidv4(), "bookName": bookName, "amazonUrl": amazonUrl, "imageName":"book1.jpg"});
  books = JSON.stringify(groceriesInfoJSON);
...
app.post('/updateBook/:id', (req, res) => {
  ...
  var amazonUrl = req.body["amazonUrl"];
  for (var i=0; i < groceriesInfoJSON.length; i++){
    if(groceriesInfoJSON[i]["id"] == bookId){
      groceriesInfoJSON[i]["bookName"] = bookName;
      groceriesInfoJSON[i]["amazonUrl"] = amazonUrl;
    }
  }
  books = JSON.stringify(groceriesInfoJSON);
  fs.writeFileSync('json/books.json', books);
...
app.get('/deleteBook/:id', (req, res) => {
  var bookId = req.params.id;
  groceriesInfoJSON = groceriesInfoJSON.filter(function( obj ) {
